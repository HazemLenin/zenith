...
import { db } from "../db";
import { sessions, skillCategories, skills, skillTransfers, studentSkills, users } from "../models";
...
export class SkillTransfersController {
    static async requestSkillTransfer(req: Request, res: Response): Promise<void> {
        try {
            const { studentId, skillId, teacherId } = req.body;
            console.log(typeof(studentId))

            // Validate input
            if (typeof(studentId) != 'number' || typeof(skillId) != 'number' || typeof(teacherId) != 'number') {
    ...
    static async getMyRequests(req: Request, res: Response): Promise<void> {
        try {
            const studentId = req.user.id; // Assuming the student ID is stored in req.user
            ...
            const requests = await db
                ...
                .where(and(
                            eq(skillTransfers.studentId, studentId),
    ...
    static async rejectRequest(req: Request, res: Response): Promise<void> {
        try {
            ...
            await db.delete(skillTransfers).where(eq(skillTransfers.id, skillTransferId));
    ...
    static async acceptRequest(req: Request, res: Response): Promise<void> {
        try {
            ...
            await db.update(skillTransfers)
            ...
            const transferDetails = await db
                ...
                .then(async results => {
                    ...
                    const sessionsList = await db
                        ...
                        .then(sessionResults => sessionResults.map(session => ({
                            id: session.id,
                            title: session.title,
                    ...
                    const completedSessionsCount = sessionsList.filter(session => session.completed).length;
        ...
        try {
           ...
           if (!Number(skillTransferId) || !Number(sessionId)) {
            ...
            await db.update(sessions)
                .set({ paid: true })
                .where(and(
                    eq(sessions.id, Number(skillTransferId)),
                    eq(sessions.skillTransferId, Number(sessionId))
                ));
