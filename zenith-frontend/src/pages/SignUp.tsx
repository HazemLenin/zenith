...
const SignUp: React.FC = () => {
  ...
  const validationSchema = Yup.object().shape({
    ...
    role: Yup.string()
      ...
      .oneOf(['student', 'instructor'], 'Role must be either student or instructor'),
  });
  ...
  const formik = useFormik<FormValues>({
    ...
    onSubmit: async (values: FormValues) => {
      setIsLoading(true);
      try {
        const { data } = await axios.post<ApiResponse>('http://localhost:3000/api/auth/signup', values);
        try {
        ...
        if (axios.isAxiosError(error)) {
          setErrorMessage(
            ...
            (error.request ? 'Network error. Please check your connection.' : 'An error occurred. Please try again later.')
          );
        } else {
